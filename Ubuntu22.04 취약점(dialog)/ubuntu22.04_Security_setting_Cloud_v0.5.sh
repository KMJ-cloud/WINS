...
function DialogSetup()	
{
    ...
    if [ -z "$dialogAMD" ]
        then
        # 경로 확인 필요
        # dpkg --force-all -i $(pwd)/../2.AMD/4.dialog/dialog_1.3-20211214-1_amd64.deb  
        dpkg --force-all -i dialog_1.3-20211214-1_amd64.deb  
...
function PwdComplexityExcute()
{
	...
	then
		...
		sed -i'' -r -e "/password\s+requisite\s+pam_deny.so/a\password\trequisite\t\t\tpam_pwquality.so enforce_for_root retry=3 minlen=8 dcredit=-1 ucredit=-1 lcredit=-1 ocredit=-1" "/etc/pam.d/common-password"
...
function AccountLockCritical()
{
	...
	echo " *suggest: auth        required			pam_faillock.so deny=5 unlock_time=120 no_magic_root\n" >> ./.SecurityInfo
	echo " *current: ${cmp1}\n\n" >> ./.SecurityInfo
    	Progress=16
...
function AccountLockCriticalExcute()
{
	...
	then
		filebackup common-auth /etc/pam.d/common-auth "02.[U-03|Account Lock-1    ] :"

		# line=$(egrep -n "auth" /etc/pam.d/common-password | egrep required | egrep pam_deny.so | grep -v '#' | awk -F ":" '{print$1}')"s"
		line=$(egrep -n "auth" /etc/pam.d/common-auth | egrep required | egrep pam_permit.so | grep -v '#' | awk -F ":" '{print$1}')"s"
...
#U-06 : ubuntu 22.04는 wheel 그룹이 아닌 sudo에 유저 추가
function wheelgroup()
{
	ret=0
